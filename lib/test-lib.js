...
const compareElements = (actual, expected, preface) => {
  ...
  describe(
    ...
    () => {
      ...
      const expectedDirectTextContent = Array.from(expected.childNodes).reduce(
        computeDirecTextContent,
        ''
      );
      if (
      ...
      for (const key in actual) {
        ...
        if (key === 'onmozfullscreenchange' || key === 'onmozfullscreenerror') {
          continue;
        }
      ...
      for (const prop of propertiesToTest) {
        ...
        if (Object(baseLineValue) === baseLineValue) {
          ...
          isBaselineValue =
            actual[prop] === baseLineValue && expected[prop] === baseLineValue;
        }
